...
LANGUAGE_ICONS = {
    ...
    'Swift': 'swift.png', 'Java': 'java.png', 'C': 'c.png',
    'Rust': 'rust.png', 'Go': 'go.png', 'HTML': 'html.png',
    'CSS': 'css.png', 'Kotlin': 'kotlin.png', 'Dart': 'dart.png',
    'Shell': 'shell.png', 'Jupyter Notebook': 'jupyter.png',
}
...
def stat_icon(kind, x, y):
    ...
    href = image_data_uri(PICKAXE_IMAGE, 'image/png')
    return (f'<image data-icon="diamond-pickaxe" aria-label="diamond pickaxe" '
            f'class="pixel-icon" x="{x}" y="{y}" width="28" height="28" '
            f'preserveAspectRatio="xMidYMid meet" href="{href}"/>')


def language_icon(font, language, x, y):
    """Embed the real language logo asset instead of approximating it in code."""
    asset_name = LANGUAGE_ICONS.get(language)
    if asset_name:
        href = image_data_uri(LANGUAGE_ICON_DIR / asset_name, 'image/png')
        return (f'<image data-language-icon="{escape(language)}" '
                f'aria-label="{escape(language)} logo" x="{x}" y="{y}" '
                f'width="30" height="30" preserveAspectRatio="xMidYMid meet" href="{href}"/>')
